...
public class DownstreamTokenProvider : IDownstreamTokenProvider
{
    ...
    public DownstreamTokenProvider(IConfiguration configuration, IAccessTokenProvider tokenProvider)
    {
    ...
    public async Task<string?> TryGetToken()
    {
        ...
        if (!accessTokenResult.TryGetToken(out var token)) return null;
